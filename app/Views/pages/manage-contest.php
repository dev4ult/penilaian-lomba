...
<header class="pb-10 border-b-2 flex justify-between">
    <div class="prose">
        ...
        <p>Pengelolaan data Perlombaan untuk pengaturan terhadap nilai yang diberikan juri terhadap peserta lomba yang
            terdaftar pada lomba</p>
    </div>
...
<div class="flex gap-3 justify-between my-10">
    ...
    <form action="/contests" method="post" class="join ml-auto">
        ...
        <div>
            <input class="input input-bordered join-item" name="search-keyword" placeholder="Cari Lomba"
...
<div class="grid grid-flow-row grid-cols-4 gap-5">
    ...
    <div class="bg-white rounded-lg border-2 hover:shadow overflow-hidden">
        <img src="<?= $contest['picture'] ? 'data:image/jpg;base64,' . base64_encode($contest['picture']) :
                            base_url('./images/default-card/' . rand(1, 4) . '.png') ?>" class="w-full h-52"
            alt="default" />
        <div class="p-6">
            <div class="prose">
            ...
            <a href="/contest/<?= $contest['contest_id'] ?>"
                class="btn btn-neutral btn-sm capitalize mt-3 text-right">lihat detail</a>
        </div>
    </div>
    <?php endforeach ?>
</div>
