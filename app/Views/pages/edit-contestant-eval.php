...
<header class="pb-10 border-b-2 flex justify-between">
    <div class="prose ">
        ...
        <p>Penilaian Peserta dilakukan untuk memberikan penilaian terhadap peserta yang sedang atau sudah melakukan
...
<div class="my-10">
    ...
    <input type="number" id="total-categories" name="total-categories" value="<?= count($categories) ?>" class="hidden" />
    ...
    <form action="/contest/contestant-evaluation/put" method="post">
        ...
        <input type="number" id="contestant-id" name="contestant-id" value="<?= $contestant['contestant_id'] ?>" class="hidden" />
        <?php foreach ($categories as $index_category => $category) : ?>
            <?php $index = 0 ?>
            <?php $category_id = $category['eval_category_id']  ?>

            <!-- Category Container -->
            <div id="category-container-<?= $category_id  ?>" class="my-6 category-container <?= $index_category != 0 ? 'hidden' : '' ?>">
                <?php $index_sub = 0 ?>
        ...
        <div class="flex gap-3 justify-between">
            ...
            <button type="submit" id="submit-btn" class="btn btn-warning capitalize hidden ml-auto">submit / simpan
                penilaian</button>
        </div>
    </form>
</div>
