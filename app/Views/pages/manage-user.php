...
<div class="flex gap-3 justify-between mb-6 mt-8">
    ...
    <!-- Search Input -->
    <form action="/users" method="post" class="join">
        <a href="/users" class="btn btn-primary capitalize btn-outline mr-3">
            refresh
        </a>
        <div class="join-item">
            <input class="input input-bordered join-item" name="search-keyword" placeholder="Cari Admin atau Juri" value="<?= isset($search_keyword) ? $search_keyword : ''  ?>" />
        </div>
        <select class="select select-bordered join-item" name="filter-role">
            <option value="" disabled selected>Filter</option>
            <option value="admin" <?= isset($filter_role) && $filter_role == 'admin' ? 'selected' : '' ?>>Admin</option>
            <option value="juri" <?= isset($filter_role) && $filter_role == 'juri' ? 'selected' : '' ?>>Juri</option>
        </select>
        <div class="indicator">
...
<dialog id="edit_modal" class="modal">
    <form id="form-edit-user" action="/user/put" method="post" class="modal-box max-w-2xl p-8 relative">
        ...
        <div id="edit-container" class="grid-flow-row grid-cols-2 gap-6 hidden">
            <input type="text" id="user-id" name="user-id" class="hidden">
        ...
        <div class="modal-action my-0">
            <button type="button" id="close-edit-modal" onclick="edit_modal.close()" class="absolute top-0 right-0 m-8 btn btn-sm btn-square btn-outline">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
